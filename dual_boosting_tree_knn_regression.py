...
class dual_boosting_regression():
    ...
    def __init__(self, n_estimator=10, max_depth=1, K=1, learning_rate=0.1, labda=1):
    ...
    def fit(self, X, y):
        ...
        boosted_predict_knn = np.zeros(len(y))
        for k in range(self.n_estimator):
            ...
            if k == 0:
                ...
                if np.sum((y-boosted_predict_tree)**2) - np.sum((y-boosted_predict_knn)**2) > 0:
                    I = 1
            ...
            boosted_res_tree = y - boosted_predict_tree + I*self.labda*(boosted_predict_knn - boosted_predict_tree)
            ...
            if k == 0:
                ...
                boosted_predict_knn = self.learning_rate*np.sum(np.array([t.predict(X) for t in self.list_knns]), axis=0)

            boosted_res_nn = y - boosted_predict_knn + (1-I)*self.labda*(boosted_predict_tree - boosted_predict_knn)

            # build new nn
            nn = KNeighborsRegressor(n_neighbors=self.K)
            nn.fit(X, boosted_res_nn)
            self.list_knns.append(nn)
    ...
    def predict_nn(self, X):
        list_nn_predict = self.learning_rate*np.sum(np.array([nn.predict(X) for nn in self.list_knns]), axis=0)
